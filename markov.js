...
(function() {
  var bodies, fs, generateChain, getNextWord, headers, markov, minSentenceLength, posts,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  fs = require('fs');

  posts = JSON.parse(fs.readFileSync("corpus2.json", "utf8"));
  ...
  headers = generateChain(posts.map(function(p) {
    return p.name;
  }));

  bodies = generateChain(posts.map(function(p) {
    return p.body;
  }));
  ...
  console.log(markov(bodies, 2000));
